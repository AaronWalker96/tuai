...
rabbits-own [age]                     ;; every rabbit has an age
foxes-own   [energy]                  ;; every fox has an energy value
...
to setup-rabbits
  ...
  [ set color white
    ...
    set shape "rabbit"                  ;; ... a random x & y coordinate
  ]
end
...
to go
  move-rabbits
  move-foxes
  if not any? turtles [ stop ]        ;; stop when no rabbits or foxes 
  tick                                ;; update screen graphics
end

to move-rabbits
  ask rabbits
  [ if age >= max-rabbit-age [ die ]    ;; if rabbits are too old they die
    set age (age + 1)
    ...
    if (trigger rabbit-breed-prob%)     ;; like rolling a big dice, hatch a new ...
    [ hatch-rabbits 1                   ;; ... rabbit if the dice rolls a zero  ...
      [ set age 0
        setxy random-xcor random-ycor
      ]
    ]
...
to move-foxes
  ask foxes
  [ if energy < 0 [ die ]              ;; if foxes run out of energy they die
    set energy (energy - 1)
    
    if not any? rabbits [ stop ]
    face nearest-of rabbits            ;; face nearest rabbit
    wiggle                             ;; randomly turn a bit
    forward 0.8                        ;; move forward a step & a half
    
    if any? rabbits-here               ;; if this fox is on a rabbit ...
    [                                  ;; ... eat it & gain energy ...
      set energy (energy + fox-energy-gain-from-rabbit)     
      ask rabbits-here [die]           ;; ... and the rabbit dies
    ]
                                       ;; if foxes have enough energy
    if energy > fox-breed-energy + fox-start-energy
    [ hatch-foxes 1                    ;; they hatch a new fox ...
      [ set energy fox-start-energy
        setxy random-xcor random-ycor
      ]
      set energy fox-start-energy      ;; ... and their energy reduces
    ]
  ]
end
@#$#@#$#@
