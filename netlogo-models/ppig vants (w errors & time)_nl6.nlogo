...
to setup4
  ...
  ask patches
    [ set pcolor white ]
  foreach [[-60 60] [50 57] [55 -47] [-50 -60]]
  [ x -> create-ants 1
...
to go
  if ticks = 9000 [tick stop]

  if ants-make-errors?
  [ ask ants
    [ if (trigger ant-error-rate%)
      [ right one-of [90 270]
  ...
  ask ants
  [ flip-tile                 ; flip the tile over
    forward 1                 ; move forward onto the next tile
    ifelse pcolor = white     ; if the tile the ant is on now is white...
    [ right 90 ]              ;     turn right
    [ left 90  ]              ; otherwise, if it's black, turn left
  ]
  tick
  wait pause-time / 1000
end


to flip-tile
  set pcolor (ifelse-value (pcolor = white) [black] [white])
end


to-report trigger [p%]
  let #m 10000
  report (random (#m * 100) < (#m * p%))
end
@#$#@#$#@
GRAPHICS-WINDOW
220
...
This model is based on a similar model from Uri Wilensky available from the NetLogo models library.
